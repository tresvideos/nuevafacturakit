...
const Button = ({children, className="", variant="primary", ...p}: any) => {
  ...
  const v:any={
    ...
    success:"bg-emerald-500 text-slate-900 hover:bg-emerald-400 focus:ring-emerald-500/20"
  };
    ...
    <header className="sticky top-0 z-40 w-full bg-white/80 backdrop-blur supports-[backdrop-filter]:bg-white/60 border-b border-slate-200">
      <Container>
        <div className="flex h-16 items-center justify-between">
          <button type="button" className="flex items-center gap-3 cursor-pointer" onClick={() => onGoto("home")} aria-label="Ir al inicio">
            <Image src="/logo.svg" alt="Facturakit" width={40} height={40} priority className="rounded-lg"/>
          ...
          <nav className="flex items-center gap-2">
            ...
            {user ? (
              <Button onClick={() => onGoto("dashboard")} className="bg-emerald-500 hover:bg-emerald-400 text-slate-900">Panel</Button>
            ) : (
              <Button onClick={() => onGoto("login")} className="bg-emerald-500 hover:bg-emerald-400 text-slate-900">Login</Button>
            )}
          </nav>
        </div>
      </Container>
...
function Home({ onGoto }: any) {
  return (
    <>
        ...
        <Container className="relative py-16 sm:py-20">
          <div className="grid items-center gap-10 lg:grid-cols-2">
            <div className="text-white">
              ...
              <p className="mt-4 text-lg text-slate-300">Selecciona una plantilla, edita desde cualquier dispositivo y descárgala en PDF o HTML. Rápido, bonito y sin complicaciones.</p>
              <ul className="mt-5 space-y-2 text-slate-300 text-sm">
                <li>• Plantillas con estructura real (no solo colores).</li>
              ...
              <div className="mt-6 flex items-center gap-3 rounded-xl bg-white/10 px-3 py-2 backdrop-blur">
                ...
                <div className="flex items-center gap-1 text-emerald-300">{Array.from({length:5}).map((_,i)=>(<span key={i}>★</span>))}</div>
            ...
            <div className="relative">
              <Image src="/hero-invoice.svg" alt="Ejemplo de factura" width={900} height={600} className="w-full rounded-2xl border border-slate-700/40 shadow-2xl"/>
      ...
      <section className="bg-white">
        <Container className="py-16">
          ...
          <div className="mt-8 relative">
            <div className="no-scrollbar flex gap-6 overflow-x-auto pb-2 snap-x">
              {TEMPLATES.map(t=>(
                <div key={t.id} className="w-[300px] shrink-0 snap-start">
                  <div className="rounded-2xl border border-slate-200 bg-white p-3 shadow-sm">
                    {/* Miniatura estable: escala controlada y alto fijo */}
                    <div className="h-48 overflow-hidden rounded-lg bg-white">
                      <div className="origin-top-left scale-[0.72] -translate-y-4 -translate-x-4 w-[860px]">
      ...
      {/* Por qué escogernos (con ticks de color) */}
          ...
          <div className="grid gap-10 lg:grid-cols-2">
            <div>
              ...
              <ul className="mt-4 space-y-3 text-slate-700 text-sm">
                {[
                  "Plantillas con jerarquía tipográfica y espaciado perfecto",
                  "Editor con descuentos %, importes, colores y logo",
                  "Exportación a PDF (print) y HTML",
                ].map((li)=>(
      ...
      <section className="bg-white">
        <Container className="py-14 text-center">
          <h4 className="text-xl font-semibold">Conecta con nosotros</h4>
          <p className="mt-2 text-slate-600">Síguenos y entérate de novedades, plantillas y consejos.</p>
          <div className="mt-6 flex items-center justify-center gap-5 text-2xl text-slate-700">
...
function Footer(){ return (<footer className="border-t border-slate-200 py-10"><Container><div className="flex flex-col items-center justify-between gap-4 sm:flex-row"><div className="flex items-center gap-3"><Image src="/logo.svg" width={24} height={24} className="rounded" alt="logo"/><p className="text-sm text-slate-600">© {new Date().getFullYear()} Facturakit. Todos los derechos reservados.</p></div><div className="flex items-center gap-4 text-xl"><a className="text-slate-600 hover:text-slate-900" href="#" aria-label="Twitter">🐦</a><a className="text-slate-600 hover:text-slate-900" href="#" aria-label="Instagram">📸</a><a className="text-slate-600 hover:text-slate-900" href="#" aria-label="LinkedIn">💼</a><a className="text-slate-600 hover:text-slate-900" href="#" aria-label="Web">🌐</a></div></div></Container></footer>);}
